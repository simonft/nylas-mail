...
var bitmask = function(){
    ...
    const auttokenh_path = path.join(app.getPath('home'), '.config', 'leap', 'authtoken');
    readFile(authtoken_path, 'utf8', function (err, data) {
        if (err) return console.log(err);
        api_token = data.replace(/(\n|\r)+$/, '');
    });
    ...
    return {
        ...
        keys: {
            ...
            list: function(uid, priv) {
                console.log('test');
                console.log(call(['keys', 'list', uid, private_str(priv)]));
